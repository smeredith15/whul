...
# --- a season laid along the days it was earned on ---------------------------

def _palmer_feed():
    """A club footballer as ESPN's squad endpoint gives him: one row for the
    year, four matches, four goals, and not a date anywhere on it."""

    class Feed:
        key, league, asset_type = "soccer-players-test", "Club Soccer", "Player"
        windowed, dated_by_source, cumulative = False, False, False
        produces, roster_scoped, live, seasons_for, accumulates = (), False, None, None, ()
        dates_from = staticmethod(lambda raw, seasons: {
            "Cole Palmer": ["2026-08-22", "2026-08-30", "2026-09-13", "2026-09-17"],
        })

        @staticmethod
        def build():
            return (lambda _seasons: pd.DataFrame([
                {"season": 2026, "player": "Cole Palmer", "player_id": "1",
                 "team": "Chelsea", "league": "Premier League",
                 "matches": 4.0, "goals": 4.0}]),
                    lambda raw, as_of=None: raw.assign(total_points=23.0))
    return Feed


def test_a_season_with_no_dates_is_laid_along_the_days_it_was_earned_on():
    """Thirty-nine footballers' seasons arrived on the fifth of September
    carrying 360 normalized points between them, because that was the first
    morning the feed was ever pulled. The squad endpoint gives the statistics
    and no days; the gamelog gives the days and no statistics; between them a
    total can be put where it happened."""
    from datetime import date as _date
    ...
    Feed = _palmer_feed()
    seen = {}
    for day in ("2026-08-21", "2026-08-25", "2026-09-01", "2026-09-14",
                "2026-09-17"):
        got = ing._pull(Feed(), _date.fromisoformat(day), verbose=False,
                        today=_date(2026, 9, 17))
        seen[day] = 0.0 if got.empty else float(got["total_points"].iloc[0])

    # Nothing before his first match; a quarter of it after one; and the whole
    # of it on the last day the feed's own figure is exact.
    assert seen["2026-08-21"] == 0.0
    assert seen["2026-08-25"] == pytest.approx(23.0 * 1 / 4)
    assert seen["2026-09-01"] == pytest.approx(23.0 * 2 / 4)
    assert seen["2026-09-14"] == pytest.approx(23.0 * 3 / 4)
    assert seen["2026-09-17"] == pytest.approx(23.0)
    # And it only ever rises, which is the invariant everything else rests on.
    assert list(seen.values()) == sorted(seen.values())


def test_the_estimate_says_that_it_is_one():
    """Split evenly, which is the part that is a guess: a hat-trick and a quiet
    twenty minutes are priced the same. The dates are not a guess and neither
    is the total on the last of them."""
    from datetime import date as _date

    from whul import ingest as ing

    got = ing._pull(_palmer_feed()(), _date(2026, 9, 1), verbose=False,
                    today=_date(2026, 9, 17))
    assert float(got["estimated_share"].iloc[0]) == 0.5
    # The counting stats move with it, so the row still tells one story.
    assert float(got["matches"].iloc[0]) == 2.0

    # On the day the feed is exact, nothing is estimated at all.
    whole = ing._pull(_palmer_feed()(), _date(2026, 9, 17), verbose=False,
                      today=_date(2026, 9, 17))
    assert "estimated_share" not in whole.columns


def test_a_player_the_gamelog_does_not_know_is_left_out_rather_than_guessed():
    """Dating him from nothing would put a season's points on the league year's
    first morning, which is the fault being fixed rather than a different way
    of committing it."""
    from datetime import date as _date
    ...
    Feed = _palmer_feed()
    Feed.dates_from = staticmethod(lambda raw, seasons: {"Somebody Else": ["2026-08-22"]})
    notes: list[str] = []
    got = ing._pull(Feed(), _date(2026, 9, 1), verbose=False,
                    today=_date(2026, 9, 17), notes=notes)
    assert got.empty
    assert notes and "no dated appearances" in notes[0]


def test_a_feed_that_can_find_its_dates_is_not_refused_a_replay():
    """The refusal is for a feed that cannot say what was true on a day that
    has gone. One that knows where to look can."""
    from datetime import date as _date

    from whul import ingest as ing

    assert ing._can_answer_for(
        pd.DataFrame([{"player": "x", "matches": 1}]),
        _date(2026, 9, 1), _date(2026, 9, 17)) != ""
    assert ing._can_answer_for(
        pd.DataFrame([{"player": "x", "matches": 1}]),
        _date(2026, 9, 1), _date(2026, 9, 17),
        dates_from=lambda *a: {}) == ""
